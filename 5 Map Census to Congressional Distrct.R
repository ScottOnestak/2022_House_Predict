...
if(theYear < 2020){
  census = census %>% left_join(.,area_2010 %>% select("id","LAND"),by="id") %>% 
  ...
  census = census %>% left_join(.,area_2020 %>% select("id","LAND"),by="id") %>% 
              mutate(ppsm = total_population / LAND) %>%
              mutate(class = ifelse(ppsm > 2500,"Highly Dense",
                                    ifelse(ppsm > 1000,"Dense",
                                           ifelse(ppsm > 500,"Spread",
...
if(theState=="AK"){
  ...
  census_update = census %>% filter((county==53 & substr(id,6,11) %in% c("030101","030102","030103","030200","030300",
  ...
  census_update = census_update %>%
                    ...
                    mutate(id=paste(state,
                                    ...
                                    formatC(as.numeric(blockgroup),width=1,format="d",flag=0),
                                    sep=""))
  census_keep = census %>% filter((!county==53  | !substr(id,6,11) %in% c("030101","030102","030103","030200","030300",
                                                         "030401","030403","030600","030402")) &
                                  (!county==65 | !substr(id,6,11) %in% c("024800","024700","024900")) &
                                  (!county==85 | !substr(id,6,11) %in% c("008900") | blockgroup==0))
...
if(theYear < 2020 & curr_year < 2020){
  ...
  if(!all(setdiff(unlist(unique(census %>% filter(year < 2020) %>% select(id))),
    ...
    holder = curr_mapping %>% 
                      ...
                      mutate(name = ifelse(is.na(GEOID_BLKGRP_20),id,GEOID_BLKGRP_20),
...
if(curr_problem == FALSE){
  curr_census = theMapping %>% left_join(.,census %>% filter(year == curr_year) %>% 
  ...
  summary = summary1 %>% left_join(.,summary2,by="CD") %>%
